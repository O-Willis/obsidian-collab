...
export enum FileSuggestExportMode {
	MdFiles,
	HtmlFiles,
}
...
export class FileSuggest extends TextInputSuggest<TFile> {
	constructor(public inputEl: HTMLInputElement, private input_mode?: FileSuggestExportMode | FileSuggestExportMode.MdFiles) {
		super(inputEl);
	}
	...
	getSuggestions(input_str: string): TFile[] {
		...
		all_files.forEach((file: TAbstractFile) => {
			const extension_compare = this.input_mode? 'html' : 'md';
			if (
				file instanceof TFile &&
				file.extension === extension_compare &&
